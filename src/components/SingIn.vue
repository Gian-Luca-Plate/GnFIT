...
export default {
    ...
    methods: {
        async LoginAccWithGitHup() {
            try {
                // GitHub OAuth Login
                const {data, error} = await supabase.auth.signInWithOAuth({
                    provider: 'github',
                    options: {
                        redirectTo: `https://fwxajlxqkuhnyvclyjak.supabase.co/auth/v1/callback`,
                    },
                });
                ...
                // Nach erfolgreicher Anmeldung: Benutzerinformationen abrufen
...
<template>
    <div id="login">
        <img src="@/assets/Log.png" class="logoForms">
        <div style="font-size: 16px; font-weight: bold">Login on an account</div>
        <br>
        <div class="FormsSing">
            <input type="email" v-model="email" class="inputForms" placeholder="example@domain.com"/>
            <input type="password" v-model="password" class="inputForms" placeholder="ABc123@!"/>
            <router-link to="/" style="text-decoration: none">
                <button type="button" @click="loginAcc" class="createButton">Login</button>
            </router-link>
        </div>
        <br>
        ...
        <br>
        <button class="custom-button" @click='LoginAccWithGoogle'>

            <svg xmlns="http://www.w3.org/2000/svg" width="40px" height="40px" viewBox="0 0 32 32" data-name="Layer 1"
                 id="Layer_1" class="logo">
                <!-- Hintergrund in #EEEEEE -->
        ...
        <br>
        <button class="custom-button" @click='LoginAccWithGitHup'>
